...
func dispatchClaim(client *beadclient.Client, id, actor string, stdout, stderr io.Writer) (code int, handled bool) {
	bead, claimed, err := client.ClaimBead(id, actor)
	if err != nil {
		if errors.Is(err, beadclient.ErrClaimRouteUnsupported) {
			return 0, false
		}
