...
func doMailCheckTargetWithFormatObserved(mp mail.Provider, target resolvedMailTarget, inject bool, hookFormat string, stdout, stderr io.Writer, observation *mailInjectionObservation) int {
	...
	if inject {
		if len(messages) > 0 {
			...
			injectedBody := formatInjectOutput(messages)
			if observation != nil {
				observation.mailIDs = make([]string, 0, len(injectedMessages))
				for _, message := range injectedMessages {
					observation.mailIDs = append(observation.mailIDs, message.ID)
				}
				observation.messageCount = len(injectedMessages)
				observation.bodyBytes = len([]byte(injectedBody))
			}
			if err := writeProviderHookContextForEvent(stdout, hookFormat, "UserPromptSubmit", injectedBody); err != nil {
				if observation != nil {
					observation.outcome = continuationOutcomeFailed
