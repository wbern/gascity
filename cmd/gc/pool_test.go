...
// TestEvaluatePoolNewDemandFilteredMatchesWorkerReadiness pins the count<->claim
// symmetry fix for the codex furiosa no_work spawn-loop (gci-x8zo / gc2-nvf76).
// The reconciler's demand probe returns routed candidate rows exactly as the
// worker's claim tier sees them; the default bead-backed path must count only
// the rows the worker would actually claim (i.e. those surviving
// filterUnreadyHookCandidates). A routed+unassigned row that bd ready returned
// but whose denormalized projection reads blocked/deferred/closed must NOT count
// as demand — else the reconciler spawns a worker that strips it, gets no_work,
// drain-acks, and respawns forever.
func TestEvaluatePoolNewDemandFilteredMatchesWorkerReadiness(t *testing.T) {
	agent := &config.Agent{
		Name:              "polecat",
		Dir:               "crm",
		BindingName:       "gastown",
	...
	sp := scaleParamsForBeads(agent, config.BeadsConfig{})
	if strings.TrimSpace(sp.NewDemandRowsCheck) == "" {
		t.Fatal("default pool agent must carry a NewDemandRowsCheck rows query")
	}

	// Two routed, unassigned candidates. One is clean (claimable); one carries a
	// denormalized self-blocked projection that bd ready still returned. The
	// worker strips the blocked one, so demand must be 1, not 2.
	rows := `[` +
		`{"id":"crm-clean","status":"open","metadata":{"gc.routed_to":"crm/gastown.polecat"}},` +
		`{"id":"crm-blocked","status":"open","is_blocked":true,"metadata":{"gc.routed_to":"crm/gastown.polecat"}}` +
		`]`
	var gotQuery string
	...
	got, err := evaluatePoolNewDemandFiltered("polecat", sp, "crm", nil, runner, time.Now())
	if err != nil {
		t.Fatalf("evaluatePoolNewDemandFiltered: %v", err)
	}
	if got != 1 {
		t.Fatalf("demand = %d, want 1 (only the clean routed row is claimable; the self-blocked row is stripped like the worker does)", got)
	}
	if gotQuery != sp.NewDemandRowsCheck {
		t.Fatalf("runner received %q, want the rows query %q", gotQuery, sp.NewDemandRowsCheck)
	}
}

// TestEvaluatePoolNewDemandFilteredFallsBackToIntForCustomScaleCheck guards that
// a custom scale_check (opaque integer, no rows form) keeps the legacy int path.
func TestEvaluatePoolNewDemandFilteredFallsBackToIntForCustomScaleCheck(t *testing.T) {
	sp := scaleParams{Min: 0, Max: 4, Check: "echo 3"} // no NewDemandRowsCheck
	runner := func(_, _ string, _ map[string]string) (string, error) { return "3\n", nil }

	got, err := evaluatePoolNewDemandFiltered("worker", sp, "", nil, runner, time.Now())
	if err != nil {
		t.Fatalf("evaluatePoolNewDemandFiltered: %v", err)
	}
	if got != 3 {
		t.Fatalf("custom scale_check demand = %d, want 3 (int path)", got)
	}
}

func TestCountClaimablePoolDemand(t *testing.T) {
	...
	cases := []struct {
		...
		{"empty array", "[]", 0, false},
		{"single ready", `[{"id":"a","status":"open"}]`, 1, false},
		{"self-blocked stripped", `[{"id":"a","status":"open","is_blocked":true}]`, 0, false},
		{"status blocked stripped", `[{"id":"a","status":"blocked"}]`, 0, false},
		{"closed stripped", `[{"id":"a","status":"closed"}]`, 0, false},
		{"dep-blocked stripped", `[{"id":"a","status":"open","blocked_by":[{"status":"open"}]}]`, 0, false},
		{"dep closed not stripped", `[{"id":"a","status":"open","blocked_by":[{"status":"closed"}]}]`, 1, false},
		{"future deferred stripped", `[{"id":"a","status":"open","defer_until":"2026-07-20T18:00:00Z"}]`, 0, false},
		{"past deferred not stripped", `[{"id":"a","status":"open","defer_until":"2026-07-20T06:00:00Z"}]`, 1, false},
		{"mixed", `[{"id":"a","status":"open"},{"id":"b","status":"open","is_blocked":true},{"id":"c","status":"open"}]`, 2, false},
		{"malformed surfaces error", `{"not":"an array"`, 0, true},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, err := countClaimablePoolDemand(tc.rows, now)
			if tc.wantErr && err == nil {
				t.Fatalf("countClaimablePoolDemand(%q) expected error", tc.rows)
			}
			if !tc.wantErr && err != nil {
				t.Fatalf("countClaimablePoolDemand(%q): %v", tc.rows, err)
			}
			if got != tc.want {
				t.Fatalf("countClaimablePoolDemand(%q) = %d, want %d", tc.rows, got, tc.want)
			}
		})
	}
}

func TestHookClaimStripDiagnostic(t *testing.T) {
	now := time.Date(2026, 7, 20, 12, 0, 0, 0, time.UTC)
	rows := `[{"id":"clean","status":"open"},{"id":"blk","status":"open","is_blocked":true},{"id":"dep","status":"open","blocked_by":[{"status":"open"}]}]`
	got := hookClaimStripDiagnostic(rows, now)
	if len(got) != 2 {
		t.Fatalf("hookClaimStripDiagnostic = %v, want 2 stripped entries", got)
	}
	joined := strings.Join(got, " | ")
	if !strings.Contains(joined, "blk (self-blocked") || !strings.Contains(joined, "dep (dependency-blocked)") {
		t.Fatalf("diagnostic missing expected reasons: %q", joined)
	}
	if strings.Contains(joined, "clean") {
		t.Fatalf("diagnostic should not include the claimable candidate: %q", joined)
	}
}
