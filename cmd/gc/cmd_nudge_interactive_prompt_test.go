...
func nudgeInteractivePromptTarget(t *testing.T, dir string, store beads.Store, fake *runtime.Fake, pane string) (nudgeTarget, string) {
	...
	// Reproduce the #2892 hazard exactly: the session LOOKS idle to WaitForIdle
	// (matchesPromptPrefix accepts an AskUserQuestion row like "> 1. Yes") while
	// a prompt is open. Without this the fake's default ErrInteractionUnsupported
	// makes every delivery fall through to the queue, so the test would pass with
	// or without the guard and prove nothing.
