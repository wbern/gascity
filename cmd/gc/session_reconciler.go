	...
	for i := range ordered {
		...
		if gateThreshold := minPositiveDuration(claimlessThreshold, claimHolderThreshold); gateThreshold > 0 && alive && sessionActivityReportable(sp, name) {
			...
			if lastActivityErr == nil && !lastActivity.IsZero() && clk.Now().Sub(lastActivity) > gateThreshold {
				...
				// Claim-holder recycle (upstream gastownhall/gascity#4012): a session
				// HOLDING an in-progress claim that has gone stale has wedged mid-work
				// on a provider condition it will not self-clear (e.g. a codex
				// "Selected model is at capacity" banner) and is invisible to every
				// other liveness surface. Opt-in via a separate, more conservative
				// [session] claim_holder_stall_timeout, since recycling a holder
				// discards in-progress work. The activity value is poke-discounted, so
				// gc's own nudges do not mask the stall. The diagnostic logs the aged
				// activity so the next wedge is captured with a real timeline.
