...
func configuredPoolWorkerNudgeBackoffActive(info sessionpkg.Info, now time.Time) bool {
	if now.IsZero() {
		return false
	}
	...
	return now.Sub(info.LastNudgeDeliveredAt) < reconcileConfiguredPoolWorkerNudgeBackoff
}
...
func reconcileSessionBeadsTracedWithNamedDemand(
		...
		if gateThreshold := minPositiveDuration(claimlessThreshold, claimHolderThreshold); gateThreshold > 0 && alive && sessionActivityReportable(sp, name) {
			...
			if lastActivityErr == nil && !lastActivity.IsZero() && clk.Now().Sub(lastActivity) > gateThreshold {
				...
				if sessionClaimHolderStalled(claimHolderThreshold, holdsClaim && !claimUnknown, providerHealthy, exempt, lastActivity, clk.Now()) {
					// Damper (gcw-8dbg): the predicate says "recycle", but a
					// recycle that has already been tried and changed nothing
	...
	for _, target := range wakeTargets {
		...
		if shouldWake && target.alive {
			...
			if isPoolManagedSessionInfo(target.info) &&
				strings.TrimSpace(target.tp.Hints.Nudge) != "" &&
				!configuredPoolWorkerNudgeBackoffActive(target.info, clk.Now()) {
				hasInProgressWork, assignedErr := sessionHasInProgressAssignedWorkForReachableStore(cityPath, cfg, store, rigStores, target.info)
				if assignedErr != nil {
					fmt.Fprintf(stderr, "session reconciler: checking in-progress assigned work before nudge for %s: %v\n", name, assignedErr) //nolint:errcheck
				} else if !hasInProgressWork {
					mgr := sessionpkg.NewManagerWithOptions(store, sp, sessionpkg.WithCityPath(cityPath), sessionpkg.WithTransportPolicyResolver(func(_ string, _ string) (string, bool) {
						if target.tp.IsACP {
							return "acp", true
						}
						return "", false
					}))
					delivered, nudgeErr := mgr.TryWaitIdleNudge(ctx, target.info.ID, reconcileConfiguredPoolWorkerNudgeSource, target.tp.Hints.Nudge, "", runtime.Config{})
					if nudgeErr != nil {
						fmt.Fprintf(stderr, "session reconciler: re-delivering configured nudge for %s: %v\n", name, nudgeErr) //nolint:errcheck
					} else if delivered {
						stampLastNudgeDeliveredAt(sessFront, target.info.ID, clk.Now())
					}
				}
			}
		}
...
func idleRespawnAckUnsafeToStop(
	...
	holdsClaim, err := sessionHasInProgressAssignedWorkForConfig(store, rigStores, info, cfg)
	if err != nil {
