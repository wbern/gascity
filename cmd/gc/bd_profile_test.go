...
func TestBdInvocationProfilerWritesProfilesAndRedactedPhaseReport(t *testing.T) {
	...
	if diff := math.Abs(report.TotalMS - report.totalPhaseDuration()); diff > 1 {
		t.Fatalf("report total_ms = %f, total phase = %f; difference %fms is diagnostic overhead, not command time", report.TotalMS, report.totalPhaseDuration(), diff)
	}
}
