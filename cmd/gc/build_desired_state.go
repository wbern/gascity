...
type DesiredStateResult struct {
	...
	// PoolNewDemandLoadVeto is the PoolNewDemandMaxLoadPercent guard's
	// decision for this tick, resolved once (see
	// resolvePoolNewDemandLoadVeto) alongside PoolNewDemandInterleaveSeed.
	// The host's 1-minute load average is a per-tick-varying input exactly
	// like the rotation seed: a caller that recomputes pool desired state
	// again on the SAME tick's inputs must reuse this decision via
	// ComputePoolDesiredStatesTracedWithSeed/ComputePoolDesiredStatesWithSeed
...
func buildDesiredStateWithSessionBeads(
	...
	if store != nil {
		...
		poolWorkBeads, poolWorkStoreRefs := filterAssignedWorkBeadsForPoolDemand(cfg, cityPath, sessionBeads.OpenInfos(), assignedWorkBeads, assignedWorkStoreRefs)
	...
	namedCanonicalInfos := make(map[string]session.Info, len(namedSpecs))
	for identity, spec := range namedSpecs {
		if info, ok := findCanonicalNamedSessionInfo(bp.sessionBeads, spec); ok {
			namedCanonicalInfos[identity] = info
		}
	}
	assigneeCanonicalIdentities := canonicalizeAssigneeIdentitiesFromInfos(namedCanonicalInfos)
...
// canonicalizeAssigneeIdentitiesFromInfos is the Info-based twin of
// session.CanonicalizeAssigneeIdentities: it maps each stale-alias assignee
// address to the canonical named-session identity that owns it (via
// alias_history), so demand matching can canonicalize a bead's stale-alias
// assignee rather than only accepting the current alias. It reuses the same
// session.MailboxAddressesFromInfo codec the mail-delivery path folds
// alias_history through, so there is ONE canonicalization instead of a second,
// divergent one on the demand-match path. An address matched by more than one
// identity's canonical session resolves to no identity at all, mirroring the
// ambiguity guard in session.CanonicalizeAssigneeIdentities.
func canonicalizeAssigneeIdentitiesFromInfos(canonicalInfos map[string]session.Info) map[string]string {
	result := make(map[string]string, len(canonicalInfos))
	ambiguous := make(map[string]bool)
	for identity, info := range canonicalInfos {
		for _, addr := range session.MailboxAddressesFromInfo(info) {
			if addr == identity {
				continue
			}
			if ambiguous[addr] {
...
func materializeProviderOverlaysBeforeFingerprint(
	...
	// and a permanent codex-hooks-drift hybrid (gcw-mnck). The runtime
