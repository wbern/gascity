...
func TestPoolIdleRoutedWorkCheckReproductionGC3Shape(t *testing.T) {
	// Demonstrates the 2026-09-14 GC3 shape: a rig store has an idle generic-ephemeral
	// pool worker (e.g. repo/worker) with no active trigger, while a task is sitting
	// open, unclaimed, and routed to repo/worker in the rig store.
	cityDir := t.TempDir()
	rigDir := t.TempDir()
	cfg := &config.City{
	...
	cityStore := beads.NewMemStoreFrom(0, nil, nil)
	rigStore := beads.NewMemStoreFrom(0, []beads.Bead{
		poolIdleWorkSessionBead("repo/worker", "active", ""),
		{
			ID:       "BEAD-GC3-STALLED",
			Title:    "repro 2026-09-14 GC3 stall task",
			Type:     "task",
			Status:   "open",
			Assignee: "",
	...
	if result.Status != doctor.StatusWarning {
		t.Fatalf("status = %v, want warning for GC3 stalled shape: %#v", result.Status, result)
	}
	details := strings.Join(result.Details, "\n")
	for _, want := range []string{"rig repo", "repo/worker", "BEAD-GC3-STALLED", "SESS-1"} {
		if !strings.Contains(details, want) {
			t.Fatalf("details missing %q:\n%s", want, details)
		}
	}
