...
// re-reading them (gcw-fisn.1: gc bd close previously paid for the same
// store.Get twice, once in the write-ID guard and once in this gate).
...
// wrapper's half of the gcw-fisn.1 dedup (evaluateWorkRecordCloseGate proves
// the preFetched-bead half above). cityPath is deliberately bogus: opening a
