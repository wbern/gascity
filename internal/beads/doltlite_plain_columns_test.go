...
// TestDoltliteReadStoreProjectsPlainColumns pins that bd's four plain columns
// reach the domain Bead from a doltlite snapshot that has them.
//
// await_type is the one field of bd's four-field gate contract
// (id/status/created_at/await_type) this store did not carry, so a gate bead
// read here reported no condition at all. created_by, owner and notes are the
// three plain-column members of the six fields bd's list/show output carries
// that Bead did not model; the other three (comment_count, dependency_count,
// dependent_count) are computed over relations and comments, not columns.
...
// and all four resolve through the same loop.
func TestDoltliteReadStoreToleratesMissingPlainColumns(t *testing.T) {
	...
	got := rows[0]
	if got.AwaitType != "" || got.CreatedBy != "" || got.Owner != "" {
		t.Fatalf("AwaitType=%q CreatedBy=%q Owner=%q, want empty when the columns are absent",
			got.AwaitType, got.CreatedBy, got.Owner)
	}
	// The rest of the row must still project, or the degraded path is not a
	// degradation but a different bug.
	if got.ID != "gate-1" || got.Title != "Gate: human" {
...
// with or without bd's await_type/created_by/owner columns.
func newPlainColumnDoltliteStore(t *testing.T, withPlainColumns bool) (*DoltliteReadStore, func()) {
	...
	if withPlainColumns {
		...
		insertVals = ", 'human', 'seeder', 'owner@example.com'"
	}
	...
	if _, err := db.Exec(`INSERT INTO issues (id, title, status, issue_type, priority, created_at, updated_at, metadata, notes` +
		insertCols + `) VALUES ('gate-1', 'Gate: human', 'open', 'gate', 2, '2026-08-01T12:00:00Z', '2026-08-01T12:00:00Z', '{}', 'waiting on a human'` +
		insertVals + `)`); err != nil {
