...
// Bead is converted to and from wire form by four hand-written mappings, and a
...
//	beadWire.toBead       (internal/beads/exec)
...
// TestBdIssueToBeadCarriesPlainColumns pins bd's four plain columns through the
...
func TestBdIssueToBeadCarriesPlainColumns(t *testing.T) {
	...
	got := issue.toBead()
	assertPlainColumns(t, "bdIssue.toBead", got, "human", "seeder", "owner@example.com", "waiting on a human")
}
...
func TestBeadFromNativeIssueCarriesPlainColumns(t *testing.T) {
	...
	assertPlainColumns(t, "beadFromNativeIssue", got, "timer", "seeder", "owner@example.com", "waiting on a timer")
}
...
func TestBeadPlainColumnsSurviveJSONRoundTrip(t *testing.T) {
	in := Bead{
		...
		AwaitType: "human", CreatedBy: "seeder", Owner: "owner@example.com", Notes: "n",
	}
	...
	for _, key := range []string{"await_type", "created_by", "owner", "notes"} {
	...
	assertPlainColumns(t, "Bead JSON round trip", out, "human", "seeder", "owner@example.com", "n")
}
...
func assertPlainColumns(t *testing.T, mapping string, got Bead, awaitType, createdBy, owner, notes string) {
	t.Helper()
