...
func (s *DoltliteReadStore) queryIssueTable(query ListQuery, tables doltliteTableSet, extraWhere string, extraArgs []any, limit int, orderBy string) ([]Bead, error) {
	...
	sqlText := `SELECT i.id, COALESCE(i.title, ''), COALESCE(i.status, ''), COALESCE(i.issue_type, ''), i.priority, i.created_at,
		...
		` + tq.flags.awaitType + `, ` + tq.flags.createdBy + `, ` + tq.flags.owner + `, ` + tq.flags.notes + `
...
type doltliteStorageFlagExprs struct {
	ephemeral string
	noHistory string
	// awaitType, createdBy, owner and notes yield bd's plain-column fields,
	// falling back to the empty-string literal on snapshots whose schema
	// predates them. They are resolved by the same presence probe as the
