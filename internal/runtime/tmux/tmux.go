...
func (t *Tmux) EnsureSessionFresh(name, workDir string) error {
	...
	// Use named session teardown; direct PID cleanup is intentionally disabled.
...
// teardown performs no direct PID signaling, so the exclusion is unnecessary.
func (t *Tmux) KillSessionWithProcessesExcluding(name string, excludePIDs []string) error {
	_ = excludePIDs
	return t.KillSessionWithProcesses(name)
}

// validProcessPID excludes sentinel/system PIDs and malformed ps output.
...
func (t *Tmux) replacePaneWindow(pane, workDir, command string) error {
	...
	if err != nil {
		t.discardPendingOwnedScope(pane)
		return fmt.Errorf("resolving replacement pane identity for %q: %w", pane, err)
	}
	t.movePendingOwnedScope(pane, newPane)
	if err := t.recordOwnedScopeStrict(newPane); err != nil {
		_, _ = t.run("kill-window", "-t", fields[0])
		return fmt.Errorf("recording replacement owned scope for pane %q: %w", newPane, err)
	}
	return nil
}
...
// selectBindingLine returns the exact key row from a full `list-keys -T` response.
func selectBindingLine(output, key string) string {
	for _, line := range strings.Split(output, "\n") {
		fields := strings.Fields(line)
		for i, field := range fields {
			if field == "-T" && i+2 < len(fields) {
				if fields[i+2] == key {
					return line
				}
				break
