// Package warmup runs warm-up-eligible doctor checks during `gc start`.
package warmup
...
type WarmupCheckResult struct { //nolint:revive // name pinned by design ga-xextj6, referenced externally as warmup.WarmupCheckResult
	Scope   string
	Check   string
...
type WarmupReport struct { //nolint:revive // name pinned by design ga-xextj6, referenced externally as warmup.WarmupReport
	StartedAt           time.Time
...
type WarmupOpts struct { //nolint:revive // name pinned by design ga-xextj6, referenced externally as warmup.WarmupOpts
	Checks           []doctor.Check
...
func RunWarmupChecks(ctx context.Context, cityPath string, cfg *config.City, opts WarmupOpts) (report *WarmupReport, err error) {
	...
	// Slice-4: a single check that owns every failure may override
	// the subject + body via CustomWarmupMail. Otherwise the slice-2
	// generic format applies.
	subject, body, ok := tryCustomSoleFailureMail(*report, checks)
	if !ok {
