...
func TestDefaultRunNetworkGitKillsDescendants(t *testing.T) {
	...
	// Allow the shim enough scheduling headroom to write its foreground
	// heartbeat before cancellation. The separate bounded-call test keeps the
	// short 300ms deadline assertion; this descendant test must first prove the
	// descendant existed or it can pass/fail without exercising group cleanup.
