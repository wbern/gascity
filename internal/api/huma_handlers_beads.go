...
// humaHandleBeadReadySummary is the bounded, server-produced projection used
// by controller discovery. It shares the authoritative federation read with
// the full ready endpoint, but applies the declared selectors before the
// compact response crosses the HTTP boundary. A partial federation read is
// rejected rather than rendered as a potentially incomplete control decision.
func (s *Server) humaHandleBeadReadySummary(ctx context.Context, input *BeadReadySummaryInput) (*IndexOutput[beads.DiscoverySummaryEnvelope], error) {
	if (input.MetadataKey == "") != (input.MetadataValue == "") {
		return nil, apierr.InvalidRequest.Msg("metadata_key and metadata_value must be supplied together")
	}
	if input.Assignee != "" && input.Unassigned {
		return nil, apierr.InvalidRequest.Msg("assignee and unassigned cannot be combined")
	}

	ready, err := s.humaHandleBeadReady(ctx, &BeadReadyInput{
	...
	if ready.Body.Partial {
		return nil, apierr.ServiceUnavailable.Msg("ready summary unavailable: federated ready read is incomplete")
	}

	limit := input.Limit
	if limit == 0 {
		limit = beads.MaxDiscoverySummaryRows
	}
	selected := make([]beads.Bead, 0, min(limit, len(ready.Body.Items)))
	for _, bead := range ready.Body.Items {
		if input.Assignee != "" && strings.TrimSpace(bead.Assignee) != input.Assignee {
			continue
		}
		if input.Unassigned && strings.TrimSpace(bead.Assignee) != "" {
			continue
		}
		if input.MetadataKey != "" && bead.Metadata[input.MetadataKey] != input.MetadataValue {
			continue
		}
		if containsString(input.ExcludeTypes, bead.Type) {
			continue
		}
		if anyStringIn(input.ExcludeLabels, bead.Labels) {
			continue
		}
	...
	return &IndexOutput[beads.DiscoverySummaryEnvelope]{
		Index:     ready.Index,
		CacheAgeS: ready.CacheAgeS,
		Body:      beads.NewDiscoverySummaryEnvelope("ready", selected, beads.DefaultDiscoverySummaryBudget),
	}, nil
}

func anyStringIn(want, values []string) bool {
