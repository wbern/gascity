...
// TestBeadFromGenCarriesPlainColumns pins bd's four plain columns through the
// generated-client mapping.
//
// This is the fourth of four hand-written Bead mappings, and the one furthest
...
func TestBeadFromGenCarriesPlainColumns(t *testing.T) {
	awaitType, createdBy := "human", "seeder"
	owner, notes := "owner@example.com", "waiting on a human"
...
func TestBeadFromGenToleratesAbsentPlainColumns(t *testing.T) {
	...
	if got.AwaitType != "" || got.CreatedBy != "" || got.Owner != "" || got.Notes != "" {
